...
impl TrainingMiner {
    ...
    pub fn with_training(config: TrainingMinerConfig) -> Self {
        ...
        Self { 
            config,
            model_trainer: Some(model_trainer),
        }
    }
    ...
    /// Generate ZK proof
    fn generate_zk_proof(&self, base_checkpoint: &Hash, loss_before: f64, loss_after: f64, training_result: &TrainingResult) -> ZKProof {
    ...
    fn select_batch_indices(&self, batch: &TrainingBatch) -> Vec<u64> {
        ...
        (0..self.config.batch_size)
            .map(|_| rng.gen_range(0..total_indices))
            .collect()
    }
