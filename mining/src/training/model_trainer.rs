...
impl ModelTrainer {
    ...
    pub fn new(input_size: usize, hidden_size: usize, output_size: usize, learning_rate: f64) -> Self {
        Self { input_size, hidden_size, output_size, learning_rate }
    }
    ...
    pub fn train_batch(&self, batch: &TrainingBatch, initial_weights: &[f32]) -> Result<TrainingResult, TrainingError> {
        ...
        let inputs = Tensor::of_slice(&batch.inputs).view([batch.batch_size as i64, self.input_size as i64]).to(Device::Cpu);

        let targets = Tensor::of_slice(&batch.targets).view([batch.batch_size as i64, self.output_size as i64]).to(Device::Cpu);
    ...
    pub fn train_batch(&self, batch: &TrainingBatch, _initial_weights: &[f32]) -> Result<TrainingResult, TrainingError> {
        ...
        Ok(TrainingResult { loss_before, loss_after, gradients, output_gradients_hash, training_time_ms: elapsed.as_millis() as u64 })
    }
...
impl TrainingJobManager {
    ...
    pub fn new(trainer: ModelTrainer) -> Self {
        Self { trainer, active_jobs: std::collections::HashMap::new() }
    }

    /// Start a new training job
    pub fn start_job(&mut self, job_id: String, model_id: String, batch: TrainingBatch) {
        let job = TrainingJob { job_id: job_id.clone(), model_id, batch, start_time: Instant::now() };
...
mod tests {
    ...
    fn test_training_batch() {
        let trainer = ModelTrainer::new(10, 20, 5, 0.001);
        let batch = TrainingBatch { inputs: vec![0.5f32; 100], targets: vec![0.3f32; 50], batch_size: 10 };
