...
use kaspa_pow::{
    genome_pow::{
        fragment_index, genome_mix_hash, GenomeDatasetLoader, GenomePowState, SyntheticLoader, GENOME_BASE_SIZE, MIX_CHUNK_BYTES,
    },
    matrix::Matrix,
    State as KHeavyState,
};
use kaspa_rpc_core::{
    api::rpc::RpcApi, model::message::GetBlockTemplateRequest, RpcRawBlock, SubmitBlockRejectReason, SubmitBlockReport,
};
...
fn maybe_set_nvidia_vulkan_icd() {
    ...
    if std::env::var_os("VK_ICD_FILENAMES").is_some() || std::env::var_os("VK_DRIVER_FILES").is_some() {
        return;
    }
...
pub async fn enumerate_mining_adapters() -> Vec<wgpu::Adapter> {
    ...
    let instance = wgpu::Instance::new(wgpu::InstanceDescriptor { backends: all_backends, ..Default::default() });
    let mut candidates: Vec<wgpu::Adapter> = instance
        ...
        .filter(|a| {
            ...
            if info.vendor == INTEL_VENDOR_ID && info.device_type == wgpu::DeviceType::IntegratedGpu {
...
pub fn select_adapters(gpu_arg: &str, adapters: Vec<wgpu::Adapter>) -> Vec<wgpu::Adapter> {
    ...
    let idx_set: std::collections::HashSet<usize> = gpu_arg.split(',').filter_map(|s| s.trim().parse::<usize>().ok()).collect();
    adapters.into_iter().enumerate().filter(|(i, _)| idx_set.contains(i)).map(|(_, a)| a).collect()
}
...
fn build_kheavy_params(pre_pow_hash: &kaspa_hashes::Hash, timestamp: u64, target: &kaspa_math::Uint256, nonce_base: u64) -> [u8; 88] {
    let mut buf = [0u8; 88];
    ...
    loop {
        ...
        if Some(template.id) != current_id {
            ...
            nonce_base = match &template.source {
                MiningSource::Stratum { extranonce1, .. } => (*extranonce1 as u64) << 32 | (gpu_id as u64 * batch_size as u64),
        ...
        let result: Option<u64> = if template.genome_active {
            let params =
                build_params_full(&template.epoch_seed, &template.pre_pow_hash, &template.target, nonce_base, template.num_mix_chunks);
            ...
            let kh_params = build_kheavy_params(&template.pre_pow_hash, template.timestamp, &template.target, nonce_base);
                ...
                Some((nonce, gpu_hash)) => {
                    match &template.source {
                        MiningSource::Node { header, .. } => {
                            ...
                            for k in 0..8 {
                                cpu_hash[k] = u32::from_le_bytes(cpu_bytes[k * 4..k * 4 + 4].try_into().unwrap());
                            }
...
pub async fn cmd_gpu(m: &ArgMatches, dash: std::sync::Arc<std::sync::Mutex<DashStats>>) {
    ...
    let file_loader: Option<kaspa_pow::genome_file::FileGenomeLoader> = genome_path.as_deref().map(|path| {
    ...
    let synthetic_bytes: Vec<u8> = if file_loader.is_none() { synthetic_packed_genome(frag_size) } else { Vec::new() };
    ...
    let (packed_genome_bytes, num_mix_chunks): (Vec<u8>, u32) = match file_loader.as_ref() {
        Some(loader) => {
            ...
            info!(
                "Genome PoW: loaded {} ({} MB, {chunks} mix-chunks)",
                genome_path.as_deref().unwrap_or(""),
                packed.len() / 1_048_576
            );
    ...
    let num_gpus = workers.len();
    {
        ...
        s.gpus = workers
            .iter()
            .map(|w| GpuStats { id: w.id, name: w.name.clone(), hashrate: 0.0, accepted: 0, rejected: 0, temp: 0, fan: 0, power: 0.0 })
            .collect();
        s.connected = true;
    ...
    let stratum_worker =
        m.get_one::<String>("stratum-worker").cloned().or_else(|| addr_str_opt.clone()).unwrap_or_else(|| "xenom-miner".to_owned());
    let stratum_password = m.get_one::<String>("stratum-password").cloned().unwrap_or_else(|| "x".to_owned());

    // ── Template watch channel (common to both node and stratum modes) ─────
    let (template_tx, template_rx) = tokio::sync::watch::channel::<Option<Arc<MiningTemplate>>>(None);
    ...
    if let Some((client, job_tx, mut job_rx, sol_rx_s)) = stratum_channels {
        ...
        let pay_address: kaspa_rpc_core::RpcAddress = Address::try_from(addr_str.as_str()).expect("Invalid --mining-address");
        let dash2 = dash.clone();
        tokio::spawn(async move {
            loop {
                match rpc.get_block_template_call(None, GetBlockTemplateRequest::new(pay_address.clone(), vec![])).await {
                    Ok(resp) => {
                        ...
                        let t = Arc::new(MiningTemplate {
                            id,
                            source: MiningSource::Node { rpc_block: Arc::new(block), header: header.clone() },
                            pre_pow_hash,
    ...
    let hash_counters: Vec<Arc<AtomicU64>> = (0..num_gpus).map(|_| Arc::new(AtomicU64::new(0))).collect();
    ...
    loop {
        ...
        loop {
            match sol_rx.try_recv() {
                Ok(sol) => {
                    handle_solution(sol, &sol_sink, &file_loader, &packed_gpu, frag_size, &dash).await;
                }
        ...
        if report_elapsed >= Duration::from_secs(5) {
            ...
            let per_gpu: Vec<f64> =
                hash_counters.iter().map(|c| c.swap(0, Ordering::Relaxed) as f64 / elapsed / 1_000_000.0).collect();
