                        ...
                        if let Ok(msg) = msg {
                            match *msg {
                                ...
                                Events::ServerStatus {
                                    ...
                                    if !is_synced {
                                        if is_open {
                                            terrorln!(this, "Unable to update the wallet state - picod node is currently syncing with the network...");

                                        } else {
                                            terrorln!(this, "picod node is currently syncing with the network, please wait for the sync to complete...");
                                        }
                                    }
...
pub async fn kaspa_cli(terminal_options: TerminalOptions, banner: Option<String>) -> Result<()> {
    ...
    let banner =
        banner.unwrap_or_else(|| format!("picod Cli Wallet v{} (type 'help' for list of commands)", env!("CARGO_PKG_VERSION")));
