...
use kaspa_rpc_core::{
    api::rpc::RpcApi, RpcAddress, RpcTransaction, RpcTransactionInput, RpcTransactionOutpoint, RpcTransactionOutput,
};
...
/// Coinbase maturity for mainnet/testnet.
pub const COINBASE_MATURITY: u64 = 100;
/// Coinbase maturity for devnet — same as mainnet per consensus params.
pub const COINBASE_MATURITY_DEVNET: u64 = 100;
...
pub async fn submit_anchor(
    ...
    let current_daa = rpc.get_block_dag_info().await.context("get_block_dag_info RPC failed")?.virtual_daa_score;

    // Collect outpoints already consumed by pending mempool txs so we don't
    // double-spend them (node returns them in the UTXO set until confirmed).
    let mempool_spent: std::collections::HashSet<(kaspa_hashes::Hash, u32)> = rpc
        .get_mempool_entries_by_addresses(vec![rpc_address.clone()], false, false)
        .await
        .unwrap_or_default()
        .into_iter()
    ...
    if utxo_entries.is_empty() {
        anyhow::bail!(
            "No mature UTXOs for funding address {rpc_address} — \
             all UTXOs may be pending in mempool (previous anchor tx unconfirmed)"
        );
    }
    ...
    // ── 4. Build change output ───────────────────────────────────────────
    //
    // The node's mempool rejects non-coinbase tx.payload AND non-standard
    // output scripts (OP_RETURN), so we use a plain self-transfer.  The anchor
    // proof is the l1_tx_ref + daa_score stored in the committer DB, which
    // maps each L2 checkpoint to an immutable L1 transaction.
    let tx_outputs = vec![TransactionOutput { value: change, script_public_key: pay_to_address_script(&rpc_address) }];

    // ── 5. Build transaction ─────────────────────────────────────────────────
    let _ = anchor_bytes; // recorded in committer DB; not embedded on-chain
    ...
    let tx_id = rpc.submit_transaction(rpc_tx, false).await.context("submit_transaction RPC failed")?;
...
pub fn keypair_from_hex(privkey_hex: &str) -> Result<Keypair> {
    ...
    let secret = SecretKey::from_str(privkey_hex).context("invalid private key — expected 64 hex chars (32 bytes)")?;
