...
async fn serve_index() -> impl IntoResponse {
    Response::builder().header(header::CONTENT_TYPE, "text/html; charset=utf-8").body(INDEX_HTML.to_owned()).unwrap()
}
...
async fn get_miner(State(s): State<ApiState>, Path(worker): Path<String>) -> Result<Json<MinerStats>, StatusCode> {
    ...
    miners.values().find(|m| m.worker == worker || m.address == worker).map(|m| Json(entry_to_stats(m))).ok_or(StatusCode::NOT_FOUND)
}
...
async fn get_blocks(State(s): State<ApiState>) -> Json<Vec<BlockRecord>> {
    if let Some(db) = &s.db {
        if let Ok(blocks) = db.get_blocks(30).await {
            ...
            for b in &blocks {
                ...
                let top = payouts.iter().max_by(|a, b| a.proportion.partial_cmp(&b.proportion).unwrap_or(std::cmp::Ordering::Equal));
    ...
    Json(
        acct.pending_payouts()
            ...
            .rev()
            .map(|p| {
                let top = p.proportions.iter().max_by(|a, b| a.1.partial_cmp(&b.1).unwrap_or(std::cmp::Ordering::Equal));
...
async fn get_payments(State(s): State<ApiState>) -> Json<Vec<TxRecord>> {
    if let Some(db) = &s.db {
        if let Ok(txs) = db.get_transactions(30).await {
            return Json(
                txs.into_iter()
                    .map(|t| TxRecord { id: t.id, tx_id: t.tx_id, submitted_at: t.submitted_at as u64, status: t.status })
                    .collect(),
            );
        }
    }
...
async fn get_network(State(s): State<ApiState>) -> Result<Json<NetworkInfo>, StatusCode> {
...
pub async fn run_api_server(listen: std::net::SocketAddr, state: ApiState) -> anyhow::Result<()> {
    let app = Router::new()
...
fn db_miner_to_stats(m: &crate::db::DbMiner, live: Option<&MinerApiEntry>) -> MinerStats {
...
pub fn update_miner_hashrate(entry: &mut MinerApiEntry, diff: f64, now_secs: u64) {
    ...
    if entry.last_share_at > 0 && now_secs > entry.last_share_at {
        ...
        entry.hashrate_hps =
            if entry.hashrate_hps > 0.0 { EWMA_ALPHA * instant + (1.0 - EWMA_ALPHA) * entry.hashrate_hps } else { instant };
    }
}
