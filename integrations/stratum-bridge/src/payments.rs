...
use kaspa_rpc_core::{
    api::rpc::RpcApi, RpcAddress, RpcTransaction, RpcTransactionId, RpcTransactionInput, RpcTransactionOutpoint, RpcTransactionOutput,
};
...
fn estimate_storage_mass(input_amounts: &[u64], output_amounts: &[u64]) -> u64 {
    ...
    let harmonic_outs: u64 =
        output_amounts.iter().map(|&v| STORAGE_MASS_PARAMETER / v.max(1)).fold(0u64, |acc, x| acc.saturating_add(x));
...
fn estimate_tx_mass(input_amounts: &[u64], output_amounts: &[u64]) -> u64 {
    let compute = BLANK_TX_MASS + input_amounts.len() as u64 * MASS_PER_INPUT + output_amounts.len() as u64 * MASS_PER_OUTPUT;
...
pub async fn execute_payout(
    ...
    let current_daa = rpc
        .get_block_dag_info()
        .await
        .map_err(|e| anyhow::Error::new(RetryablePayoutError(format!("get_block_dag_info RPC failed: {e} — will retry"))))?
        .virtual_daa_score;
    ...
    let utxo_entries: Vec<_> = rpc
        ...
        .await
        .map_err(|e| {
            anyhow::Error::new(RetryablePayoutError(format!(
                "get_utxos_by_addresses RPC failed (node needs --utxoindex?): {e} — will retry"
            )))
        })?
        .into_iter()
    ...
    if utxo_entries.is_empty() {
        return Err(anyhow::Error::new(RetryablePayoutError(format!(
            "No mature UTXOs available for pool address {pool_address} — will retry next cycle"
        ))));
    }
    ...
    let mut sorted_utxos: Vec<_> =
        utxo_entries.iter().filter(|e| !spent.contains(&(e.outpoint.transaction_id, e.outpoint.index))).collect();
    ...
    if total_pool == 0 {
        return Err(anyhow::Error::new(RetryablePayoutError(format!(
            "No spendable UTXOs for pool address {pool_address} — will retry next cycle"
        ))));
    }
    ...
    // => min_viable = C / (MAX_SELECTABLE_MASS + best_arithmetic_ins)
    {
        ...
        if n_best > 0 {
            let sum_best: u64 = sorted_utxos.iter().take(n_best as usize).map(|e| e.utxo_entry.amount).sum();
            ...
            let min_viable = if max_harmonic == 0 { u64::MAX } else { STORAGE_MASS_PARAMETER / max_harmonic };
            if min_viable > 1 {
    ...
    while out_cursor < outputs_plan.len() {
        ...
        'outer: for (addr, amount) in outputs_plan.iter().skip(out_cursor) {
            ...
            let input_amts: Vec<u64> = sorted_utxos[batch_start_utxo..temp_end].iter().map(|e| e.utxo_entry.amount).collect();
        ...
        let mut tx_outputs: Vec<TransactionOutput> = batch_outputs
            .iter()
            .map(|(addr, amount)| TransactionOutput { value: *amount, script_public_key: pay_to_address_script(addr) })
            .collect();
        if batch_change >= CHANGE_DUST_THRESHOLD {
            tx_outputs.push(TransactionOutput { value: batch_change, script_public_key: pay_to_address_script(pool_address) });
        ...
        let tx_inputs: Vec<TransactionInput> = batch_utxos
            ...
            .map(|e| TransactionInput {
                previous_outpoint: TransactionOutpoint { transaction_id: e.outpoint.transaction_id, index: e.outpoint.index },
        ...
        for i in 0..n_inputs {
            ...
            let msg = Message::from_digest_slice(sig_hash.as_bytes().as_slice()).context("secp256k1 message from sig_hash")?;
        ...
        let tx_id = rpc.submit_transaction(rpc_tx, false).await.map_err(|e| {
            anyhow::Error::new(RetryablePayoutError(format!("submit_transaction batch {batch_num} RPC failed: {e} — will retry")))
        })?;
    ...
    first_tx_id.ok_or_else(|| {
        anyhow::Error::new(RetryablePayoutError("No UTXOs available to cover any payout output — will retry next cycle".into()))
    })
}
...
pub async fn consolidate_utxos(
    ...
    let tx_outputs = vec![TransactionOutput { value: consolidate_amount, script_public_key: pay_to_address_script(pool_address) }];
    ...
    for i in 0..n_inputs {
        let sig_hash = calc_schnorr_signature_hash(&mutable_tx.as_verifiable(), i, SIG_HASH_ALL, &mut reused_values);
        let msg = Message::from_digest_slice(sig_hash.as_bytes().as_slice()).context("secp256k1 message from sig_hash (sweep)")?;
        let sig = keypair.sign_schnorr(msg);
    ...
    info!("UTXO sweep submitted: {tx_id}  inputs={}  consolidated={} sompi  fee={} sompi", n_inputs, consolidate_amount, fee);
    Ok(Some(tx_id))
}
...
fn consensus_tx_to_rpc(tx: Transaction) -> RpcTransaction {
    RpcTransaction {
        ...
        outputs: tx
            .outputs
            .into_iter()
            .map(|o| RpcTransactionOutput { value: o.value, script_public_key: o.script_public_key, verbose_data: None })
            .collect(),
