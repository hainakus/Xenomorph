...
impl Db {
    ...
    async fn migrate(&self) -> Result<()> {
        ...
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_shares_worker ON shares(worker)").execute(self.pool()).await?;

        sqlx::query("CREATE INDEX IF NOT EXISTS idx_shares_at ON shares(submitted_at)").execute(self.pool()).await?;
        ...
        let _ = sqlx::query("ALTER TABLE blocks ADD COLUMN tx_id TEXT").execute(self.pool()).await;
    ...
    pub async fn upsert_miner_connected(&self, worker: &str, address: &str, now: i64) -> Result<()> {
        sqlx::query(
    ...
    pub async fn upsert_miner_share(&self, worker: &str, now: i64, current_diff: f64, hashrate_hps: f64) -> Result<()> {
        sqlx::query(
            "UPDATE miners SET
                last_share   = ?2,
    ...
    pub async fn upsert_miner_block(&self, worker: &str) -> Result<()> {
        sqlx::query("UPDATE miners SET blocks_total = blocks_total + 1 WHERE worker = ?1").bind(worker).execute(self.pool()).await?;
        Ok(())
    }

    /// Mark a miner as disconnected; preserve all counters.
    pub async fn set_miner_disconnected(&self, worker: &str) -> Result<()> {
        sqlx::query("UPDATE miners SET connected = 0 WHERE worker = ?1").bind(worker).execute(self.pool()).await?;
        Ok(())
    }

    // ── Share operations ──────────────────────────────────────────────────────

    pub async fn insert_share(&self, worker: &str, job_id: &str, diff: f64, now: i64) -> Result<()> {
        sqlx::query("INSERT INTO shares (worker, job_id, difficulty, submitted_at) VALUES (?1,?2,?3,?4)")
            .bind(worker)
            .bind(job_id)
            .bind(diff)
            .bind(now)
    ...
    pub async fn prune_shares(&self, max_age_secs: i64, now: i64) -> Result<()> {
        sqlx::query("DELETE FROM shares WHERE submitted_at < ?1").bind(now - max_age_secs).execute(self.pool()).await?;
        Ok(())
    }

    // ── Block operations ──────────────────────────────────────────────────────

    pub async fn insert_block(&self, job_id: &str, found_at: i64, daa_score: i64) -> Result<()> {
        sqlx::query("INSERT OR IGNORE INTO blocks (job_id, found_at, block_daa_score, status) VALUES (?1,?2,?3,'pending')")
            .bind(job_id)
            .bind(found_at)
            .bind(daa_score)
            .execute(self.pool())
    ...
    pub async fn insert_block_payouts(&self, job_id: &str, proportions: &[(String, f64)]) -> Result<()> {
        for (worker, proportion) in proportions {
            sqlx::query("INSERT OR IGNORE INTO block_payouts (job_id, worker, proportion) VALUES (?1,?2,?3)")
                .bind(job_id)
                .bind(worker)
                .bind(proportion)
                .execute(self.pool())
    ...
    pub async fn update_block_status(&self, job_id: &str, status: &str, tx_id: Option<&str>) -> Result<()> {
        sqlx::query("UPDATE blocks SET status = ?2, tx_id = ?3 WHERE job_id = ?1")
            .bind(job_id)
            .bind(status)
            .bind(tx_id)
            .execute(self.pool())
    ...
    pub async fn get_block_payouts(&self, job_id: &str) -> Result<Vec<DbBlockPayout>> {
        let rows = sqlx::query("SELECT job_id, worker, proportion FROM block_payouts WHERE job_id = ?1")
            .bind(job_id)
            .fetch_all(self.pool())
            .await?;
        ...
        Ok(rows
            .iter()
            .map(|r| DbBlockPayout { job_id: r.get("job_id"), worker: r.get("worker"), proportion: r.get("proportion") })
            .collect())
    }

    pub async fn count_blocks(&self) -> Result<i64> {
        let row = sqlx::query("SELECT COUNT(*) AS n FROM blocks").fetch_one(self.pool()).await?;
        Ok(row.get("n"))
    }

    pub async fn count_shares(&self) -> Result<i64> {
        let row = sqlx::query("SELECT COUNT(*) AS n FROM shares").fetch_one(self.pool()).await?;
        Ok(row.get("n"))
    }

    pub async fn total_pool_hashrate(&self) -> Result<f64> {
        let row =
            sqlx::query("SELECT COALESCE(SUM(hashrate_hps), 0.0) AS h FROM miners WHERE connected = 1").fetch_one(self.pool()).await?;
        Ok(row.get("h"))
    }

    pub async fn count_connected_miners(&self) -> Result<i64> {
        let row = sqlx::query("SELECT COUNT(*) AS n FROM miners WHERE connected = 1").fetch_one(self.pool()).await?;
        Ok(row.get("n"))
    }
    ...
    pub async fn insert_transaction(&self, tx_id: &str, status: &str, now: i64) -> Result<()> {
        sqlx::query("INSERT INTO transactions (tx_id, submitted_at, status) VALUES (?1, ?2, ?3)")
            .bind(tx_id)
            .bind(now)
            .bind(status)
            .execute(self.pool())
