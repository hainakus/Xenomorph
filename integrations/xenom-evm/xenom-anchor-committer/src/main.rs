...
fn build_commit_payload(checkpoint_id: &[u8; 32], block_number: u64, chain_id: u64) -> [u8; COMMIT_PAYLOAD_SIZE] {
...
impl CommitterDb {
    fn open(state_dir: &Path) -> Result<Self> {
        ...
        let db = DB::open_cf_descriptors(&opts, db_path, vec![cf]).context("open committer RocksDB")?;

        Ok(Self { db: Arc::new(db) })
    }

    fn get(&self, block_number: u64) -> Result<Option<CommitRecord>> {
        let cf = self.db.cf_handle(CF_COMMIT_STATUS).ok_or_else(|| anyhow::anyhow!("missing CF {CF_COMMIT_STATUS}"))?;
        match self.db.get_cf(cf, block_number.to_be_bytes()).context("rocksdb get")? {
            Some(bytes) => {
                let rec: CommitRecord = bincode::deserialize(&bytes).context("deserialize CommitRecord")?;
                Ok(Some(rec))
            }
            None => Ok(None),
        }
    ...
    fn put(&self, rec: &CommitRecord) -> Result<()> {
        ...
        self.db.put_cf(cf, rec.block_number.to_be_bytes(), bytes).context("rocksdb put")
    }
}

// ── EVM checkpoint polling ────────────────────────────────────────────────────

async fn fetch_latest_checkpoint(http: &reqwest::Client, evm_url: &str) -> Result<Option<(L1CheckpointV1, String)>> {
    ...
    let bytes_hex =
        result["bytes"].as_str().ok_or_else(|| anyhow::anyhow!("missing 'bytes' in checkpoint response"))?.trim_start_matches("0x");

    let bytes = hex::decode(bytes_hex).context("decode checkpoint bytes hex")?;

    let checkpoint_id =
        result["checkpointId"].as_str().ok_or_else(|| anyhow::anyhow!("missing 'checkpointId' in checkpoint response"))?.to_owned();

    let cp =
        L1CheckpointV1::from_bytes(&bytes).ok_or_else(|| anyhow::anyhow!("invalid checkpoint bytes (got {} bytes)", bytes.len()))?;
...
async fn commit_cycle(
    ...
    db: &CommitterDb,
    dry_run: bool,
    pending_txid: &mut Option<kaspa_rpc_core::RpcTransactionId>,
) -> Result<bool> {
    // If the previous anchor tx is still pending in mempool, skip until it clears.
    if let (Some(txid), Some(rpc_client)) = (pending_txid.as_ref(), rpc) {
        match rpc_client.get_mempool_entry(*txid, false, false).await {
            Ok(_) => {
                log::debug!("previous anchor tx {txid} still in mempool — skipping");
                return Ok(false);
            }
            Err(_) => {
                // No longer in mempool: confirmed or dropped.
                log::info!("previous anchor tx {txid} cleared from mempool");
                *pending_txid = None;
            }
        }
    }
    ...
    let id_bytes_vec = hex::decode(checkpoint_id.trim_start_matches("0x")).context("decode checkpoint_id")?;
    let mut id_arr = [0u8; 32];
    let copy_len = id_bytes_vec.len().min(32);
    id_arr[..copy_len].copy_from_slice(&id_bytes_vec[..copy_len]);

    let commit_payload = build_commit_payload(&id_arr, block_number, checkpoint.chain_id);
    ...
    let daa_score = rpc_client.get_block_dag_info().await.context("get_block_dag_info")?.virtual_daa_score;

    let l1_tx_ref =
        xenom_anchor_client::submit_anchor(rpc_client, kp, &commit_payload, fee_sompi, prefix, coinbase_maturity).await.context("submit_anchor to L1")?;

    log::info!("  l1_tx={l1_tx_ref}  daa_score={daa_score}");
    if let Ok(txid) = kaspa_rpc_core::RpcTransactionId::from_str(&l1_tx_ref) {
        *pending_txid = Some(txid);
    }

    let rec =
        CommitRecord { block_number, checkpoint_id, l1_tx_ref, daa_score, status: "submitted".to_owned(), created_at_ms: now_ms() };
    db.put(&rec)?;
...
async fn main() -> Result<()> {
    ...
    let evm_raw = m.get_one::<String>("evm-node").cloned().unwrap_or_else(|| "127.0.0.1:8545".to_owned());
    let evm_url = if evm_raw.starts_with("http") { evm_raw } else { format!("http://{evm_raw}") };

    let node_raw = m.get_one::<String>("node").cloned().unwrap_or_else(|| "127.0.0.1:36669".to_owned());
    let node_url = if node_raw.starts_with("grpc://") { node_raw.clone() } else { format!("grpc://{node_raw}") };

    let state_dir = m.get_one::<String>("state-dir").cloned().unwrap_or_else(|| "/var/lib/xenom-evm-committer".to_owned());

    let poll_ms: u64 = m.get_one::<String>("poll-ms").and_then(|s| s.parse().ok()).unwrap_or(10_000);
    ...
    let fee_sompi: u64 =
        m.get_one::<String>("fee-sompi").and_then(|s| s.parse().ok()).unwrap_or(xenom_anchor_client::DEFAULT_FEE_PER_INPUT);
    ...
    if !dry_run && privkey_hex.is_none() {
        anyhow::bail!("--submit requires a private key: set $COMMITTER_PRIVKEY or use --key-file <PATH>");
    }
    ...
    if let Some(ref kp) = keypair {
        log::info!("  funding:   {}", xenom_anchor_client::address_from_keypair(kp, network_prefix));
    }
    ...
    let mut pending_txid: Option<kaspa_rpc_core::RpcTransactionId> = None;
...
fn cli() -> Command {
    Command::new("xenom-anchor-committer")
        ...
        .arg(Arg::new("poll-ms").long("poll-ms").value_name("MS").default_value("10000").help("Poll interval in milliseconds"))
        .arg(
            Arg::new("submit")
                .long("submit")
        ...
        .arg(Arg::new("key-file").short('k').long("key-file").value_name("PATH").help(
            "Path to file containing the secp256k1 private key (64 hex chars). \
                     Alternatively set $COMMITTER_PRIVKEY. Required with --submit.",
        ))
        .arg(Arg::new("fee-sompi").long("fee-sompi").value_name("N").default_value("2000").help("Relay fee per input in sompi"))
        .arg(Arg::new("devnet").long("devnet").action(clap::ArgAction::SetTrue).help("Use devnet address prefix (xenomdev:)"))
        .arg(Arg::new("testnet").long("testnet").action(clap::ArgAction::SetTrue).help("Use testnet address prefix (xenomtest:)"))
}
