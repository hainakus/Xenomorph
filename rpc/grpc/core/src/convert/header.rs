...
try_from!(item: &protowire::RpcBlockHeader, kaspa_rpc_core::RpcRawHeader, {
    Self {
        ...
        utxo_commitment: RpcHashBlake2::from_str(&item.utxo_commitment)?,
...
mod tests {
    ...
    use kaspa_rpc_core::{RpcBlock, RpcHash, RpcHashBlake2, RpcHeader};
    ...
    fn new_unique_blake2() -> RpcHashBlake2 {
        use std::sync::atomic::{AtomicU64, Ordering};
        static COUNTER: AtomicU64 = AtomicU64::new(1);
        let c = COUNTER.fetch_add(1, Ordering::Relaxed);
        RpcHashBlake2::from_u64_word(c)
    }
    ...
    fn test_rpc_header() {
        let r = Header::new_finalized(
            ...
            new_unique(),
            new_unique(),
            RpcHashBlake2::from_bytes(new_unique().as_bytes()),
            123,
            12345,
            98765,
            120055,
