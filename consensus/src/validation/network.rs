...
pub enum ValidationMessage {
    ...
    ValidationRequest { block_hash: [u8; 32] },

    /// Response with validation result
    ValidationResponse { block_hash: [u8; 32], validator: String, result: VerificationResult },

    /// Challenge for suspected invalid block
    SelectionChallenge { block_hash: [u8; 32], challenge_data: Vec<u8> },

    /// Broadcast of aggregated signature
    AggregatedSignatureBroadcast { block_hash: [u8; 32], aggregated_signature: AggregatedSignature },
}
...
impl SignatureAggregator {
    ...
    pub fn add_signature(&mut self, validator: String, signature: Vec<u8>) {
        let sig =
            ValidationSignature { validator: validator.clone(), signature, timestamp: Instant::now().elapsed().as_millis() as u64 };
    ...
    pub fn aggregate(&self) -> Result<AggregatedSignature, NetworkError> {
        if !self.has_quorum() {
            return Err(NetworkError::InsufficientSignatures { have: self.signatures.len(), required: self.required_signatures });
        }
...
impl ValidationNetwork {
    pub fn new(local_address: String, timeout_ms: u64, approval_threshold: f64) -> Self {
        Self { local_address, timeout_ms, approval_threshold }
    }

    pub fn process_message(&self, message: ValidationMessage) -> Result<NetworkEvent, NetworkError> {
        match message {
            ValidationMessage::ValidationRequest { block_hash } => Ok(NetworkEvent::ValidationRequested { block_hash }),
            ValidationMessage::ValidationResponse { validator, result, .. } => {
                Ok(NetworkEvent::ValidationResponseReceived { validator, result })
            }
            ValidationMessage::SelectionChallenge { block_hash, .. } => Ok(NetworkEvent::ChallengeReceived { block_hash }),
...
mod tests {
    ...
    fn test_aggregate() {
        ...
        for i in 0..3 {
            ...
            aggregator.add_validation_result(format!("validator{}", i), create_verification_result(true));
        }
    ...
    fn test_should_reject_validation() {
        ...
        for i in 3..7 {
            aggregator.add_signature(format!("validator{}", i), vec![i as u8; 32]);
            aggregator.add_validation_result(format!("validator{}", i), create_verification_result(false));
        }
